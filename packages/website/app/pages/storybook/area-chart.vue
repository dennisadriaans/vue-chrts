...
useSeoMeta({
  title: 'Nuxt Charts - AreaChart Storybook',
  description: 'Test AreaChart component with a beautiful interface.'
})
...
const hideArea = ref(false)
const lineWidth = ref(2)
const curveType = ref(CurveType.MonotoneX)
...
const chartCode = computed(() => {
  ...
  curve-type="${options.curveType}"
  :hide-area="${options.hideArea}"
    ...
    <div class="flex items-start  gap-6">
      <StorybookControls class="w-full lg:w-100 shrink-0">
        ...
        <StorybookSelectControl
          v-model="curveType"
          label="Curve Type"
          :options="Object.values(CurveType).map(v => ({ label: v, value: v }))"
        />
